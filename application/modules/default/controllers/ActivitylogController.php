...
class Default_ActivitylogController extends Zend_Controller_Action
{
    ...
    public function userlogsearchajaxAction()
    {
        if ($this->getRequest()->getPost()) {
            ...
            switch ($field) {
                case 'userfullname':
                    $where = "cc.userfullname like '%" . $searchString . "%'";
                    break;
                case 'groupname':
                    $groupIds = $this->activitylog_model->getgroupIdByString($searchString);
                    $groupIdCsv = @implode(",", $groupIds);
                    $where = "cc.group_id IN (" . $groupIdCsv . ")";
                    break;
                case 'role':
                    $empRoleIds = $this->activitylog_model->getemproleIdByString($searchString);
                    $empRoleIdCsv = @implode(",", $empRoleIds);
                    $where = "cc.emprole IN (" . $empRoleIdCsv . ")";
                    break;
                case 'emailaddress':
                    $where = "cc.emailaddress like '%" . $searchString . "%'";
                    break;
                case 'logindatetime':
                    $where = "cc.logindatetime like '%" . $searchString . "%'";
                    break;
                case 'empipaddress':
                    $where = "cc.empipaddress like '%" . $searchString . "%'";
                    break;
            }
