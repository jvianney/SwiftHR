    ...
    public function indexAction()
    {
        if (defined('EMPTABCONFIGS')) {
            ...
            if (in_array('medical_claims', $empOrganizationTabs)) {
                $userID = "";
                $conText = "";
                $searchArr = array('injury', 'disability', 'maternity', 'paternity');
                ...
                switch ($type) {
                    case 1:    //Paternity
                        ...
                        if ($amount_claimed != "" && $total_cost != "") {
                            if ($amount_claimed > $total_cost) {
                                $msgarray['amount_claimed'] = 'Amount claimed should not be greater than total cost.';
                                $errFlag++;
                            }
                        }
                        if ($amount_claimed != "" && $amount_approved != "") {
                            if ($amount_approved > $amount_claimed) {
                                $msgarray['amount_approved'] = 'Amount approved should not be greater than amount claimed.';
                                $errFlag++;
                            }
                        }
                    ...
                    case 3:    //Disability
                        ...
                        if ($injury_name == "") {
                            //Error
                            $msgarray['injury_name'] = 'Please enter disability.';
                            $errFlag++;
                        }
                        if ($insurer_name == "") {
                            $msgarray['insurer_name'] = "Please enter insurer name.";
                        ...
                        if ($disability_type == "") {
                            $msgarray['disability_type'] = 'Please select disability type.';
                            $errFlag++;
                        }
                        if ($disability_type != "" && $disability_type == "other impairments" && $other_disability_type == "") {
                            $msgarray['other_disability_type'] = 'Please enter any other disability type.';
                            $errFlag++;
                        }
                        if ($amount_claimed == "") {
                            $msgarray['amount_claimed'] = 'Please enter amount claimed for.';
                    ...
                    case 4:        //Injury
                        ...
                        if ($injury_name == "") {
                            //Error
                            $msgarray['injury_name'] = 'Please enter injury.';
                            $errFlag++;
                        }
                        if ($injury_severity == "") {
                            //Error
                            $msgarray['injury_severity'] = 'Please select injury severity.';
                            $errFlag++;
                        }
                        if ($insurer_name == "") {
                            $msgarray['insurer_name'] = "Please enter medical insurer name.";
                            $errFlag++;
                        }
                        if ($expected_date_join == "") {
                            $msgarray['expected_date_join'] = 'Please select date.';
