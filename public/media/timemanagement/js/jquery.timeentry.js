	...
	// Cross-populate minimum/maximum times for a range
 	...
 	return newTime;
        ...
        _postAttach: function (elem, inst) {
            elem.on('focus.' + inst.name, this._doFocus).on('blur.' + inst.name, this._doBlur).on('click.' + inst.name, this._doClick).on('keydown.' + inst.name, this._doKeyDown).on('keypress.' + inst.name, this._doKeyPress).on('paste.' + inst.name, function (event) { // Check pastes
                setTimeout(function () {
        ...
        _expandSpinner: function (event) {
            ...
            if (inst.options.spinnerBigImage) {
                ...
                $('<div class="' + plugin._expandClass + '" style="position: absolute; left: ' +
                    ...
                    inst.options.spinnerBigImage + ') no-repeat 0px 0px; z-index: 10;"></div>').mousedown(plugin._handleSpinner).mouseup(plugin._endSpinner).mouseout(plugin._endExpand).mousemove(plugin._describeSpinner).insertAfter(spinner);
            }
        },
        ...
        _handleSpinner: function (event) {
            ...
            if (region >= 3 && inst.options.spinnerRepeat[0]) { // Repeat increment/decrement
                ...
                $(spinner).one('mouseout', plugin._releaseSpinner).one('mouseup', plugin._releaseSpinner);
            }
        },
