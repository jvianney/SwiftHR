    ...
    var methods = {
        init: function (options) {
            this.each(function () {
                ...
                var $scrollObj = $(this).scrollTop(0).scrollLeft(0).css({'overflow': 'hidden'}).data({
                    'prev-overflow': prevOverflow,
                    'prev-float': prevFloat
                });
                ...
                if (parseInt($verticalBar.css('width')) == 0 || $verticalBar.css('width') == 'auto') $verticalBar.css({
                    'width': 7,
                    'border-radius': 3,
                    'cursor': 'default'
                });
                ...
                if (parseInt($horizontalBar.css('height')) == 0 || $horizontalBar.css('height') == 'auto') $horizontalBar.css({
                    'height': 7,
                    'border-radius': 3,
                    'cursor': 'default'
                });
                ...
                if (isTouch) {
                    ...
                    $scrollContent
                        .draggable({
                            ...
                            containment: [dragLeftCorner + holderOffsetLeft, dragTopCorner + holderOffsetTop, holderOffsetLeft, holderOffsetTop]
                        })
                        .bind('drag', function () {
                    ...
                    $scrollObj.hover(
                        function () {
                            if (settings['hide-bars']) {
                                if (verticalBarMultiplier < 1) {
                                    $verticalBar.stop(true, true).css('opacity', '').fadeIn(settings['animation-time'] / 4);
                                }
                                if (horizontalBarMultiplier < 1) {
                                    $horizontalBar.stop(true, true).css('opacity', '').fadeIn(settings['animation-time'] / 4);
                                }
                            }
                            mouseWheel(true);
                        },
                ...
                function handleSizeChanges() {
                    ...
                    if (($scrollContent.height() != scrollContentHeight) || ($scrollContent.width() != scrollContentWidth) || ($scrollObj.width() != scrollHolderWidth) || ($scrollObj.height() != scrollHolderHeight)) {
                        ...
                        $scrollContent.stop(true, false).animate({
                            'left': currentScrollLeft,
                            'top': currentScrollTop
                        }, {duration: settings['animation-time'] / 2, queue: true});
                ...
                function handleMouseWheel(event) {
                    ...
                    $scrollContent.stop(true, false).animate({'top': newContentTop}, {
                        duration: settings['animation-time'],
                        queue: true,
                        easing: settings['easing']
                    });
                    $verticalBar.stop(true, false).animate({'top': newContentTop / verticalScrollMultiplier}, {
                        duration: settings['animation-time'],
                        queue: true,
                        easing: settings['easing']
                    });
        ...
        remove: function () {
            this.each(function () {
                ...
                $scrollObj.css({
                    'overflow': $scrollObj.data('prev-overflow'),
                    'float': $scrollObj.data('prev-float')
                }).removeClass('alternate-scroll');
            });
        }
    }
