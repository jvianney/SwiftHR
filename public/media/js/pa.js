    ...
    jConfirm("Are you sure you want to delete " + groupname + " group?", "Delete group", function (r) {
        if (r == true) {
            if (appraisal_id != '' && manager_id != '' && group_id != '') {
                $.blockUI({width: '50px', message: $("#spinner").html()});
                $.post(base_url + "/appraisalmanager/deletemanagergroup", {
    ...
    jConfirm(msg, "Submit Initialization", function (r) {
        if (r == true) {
            if (appraisal_id != '' && manager_id != '') {
                ...
                $('#id_create_group').html('');
                $('#id_create_group').hide();
                $.post(base_url + "/appraisalmanager/submitmanager", {
                    appraisal_id: appraisal_id,
                    manager_id: manager_id
                }, function (data) {
                    $.unblockUI();
...
function discard_step2(init_id, management_appraisal) {
    $.blockUI({width: '50px', message: $("#spinner").html()});
    $.post(base_url + "/appraisalinit/discardsteptwo", {
        init_id: init_id,
        management_appraisal: management_appraisal
    }, function (data) {
        $.unblockUI();
...
function addlinemanager(init_id, context, line1_id, manager_levels, editid) {
    ...
    $('.acc_edit_div').html('');
    $('div[id^="idemps_"]').html('');
    $.post(base_url + "/appraisalinit/addlinemanager/format/html", {
        init_id: init_id,
        context: context,
        line1_id: line1_id,
        levels: manager_levels
    }, function (data) {
        if (context == 'edit') {
            $("#div_" + editid).show();
...
function display_managers(init_id, context, line1_id) {
    ...
    $.post(base_url + "/appraisalinit/displaymanagers/format/html", {
        type: 'line',
        levels: levels,
        init_id: init_id,
        context: context,
